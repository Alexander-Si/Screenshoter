...
namespace Screenshoter.Models
{
	public partial class ScreenshotMaker
	{
		...
		public static async void CutTheScreenAsynk(Screenshot screen, int left, int top, int wight, int height) => OnEndCreateScreenshot?.Invoke(await Task.Run(() => CutTheScreen(screen, left, top, wight, height)));

		public static async void CutTheScreenAsynk(Screenshot screen, Point location, Size size) => OnEndCreateScreenshot?.Invoke(await Task.Run(() => CutTheScreen(screen, location, size)));
	}
}
